package com.sh.model;

public class DmRyJbxx {
    private String rybh;

    private String xm;

    private String xmpy;

    private String cym;

    private String bmch;

    private String xb;

    private String csrq;

    private String gmsfhm;

    private String mz;

    private String csgj;

    private String gj;

    private String hjzzbh;

    private String jgqx;

    private String hjqh;

    private String hjxz;

    private String jwzrq;

    private String xzzbh;

    private String xzqh;

    private String xzxz;

    private String xzzrq;

    private String fwcs;

    private String zzmm;

    private String whcd;

    private Short sg;

    private String byzk;

    private String hyzk;

    private String xx;

    private String zw;

    private String zylb;

    private String sf;

    private String rylb;

    private String rybq;

    private String ywm;

    private String csxz;

    private String zjlx;

    private String zjhm;

    private String lxdh;

    private String zjxy;

    private String djrzh;

    private String djrxm;

    private String djsj;

    private String djdw;

    private String djdwmc;

    private String xgrzh;

    private String xgrxm;

    private String xgsj;

    private String xgdw;

    private String xgdwmc;

    private String sjly;

    private String zwbh;

    private String dnabh;

    private String xxjb;

    private String fwbh;

    private String fwdz;

    public String getRybh() {
        return rybh;
    }

    public void setRybh(String rybh) {
        this.rybh = rybh == null ? null : rybh.trim();
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm == null ? null : xm.trim();
    }

    public String getXmpy() {
        return xmpy;
    }

    public void setXmpy(String xmpy) {
        this.xmpy = xmpy == null ? null : xmpy.trim();
    }

    public String getCym() {
        return cym;
    }

    public void setCym(String cym) {
        this.cym = cym == null ? null : cym.trim();
    }

    public String getBmch() {
        return bmch;
    }

    public void setBmch(String bmch) {
        this.bmch = bmch == null ? null : bmch.trim();
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb == null ? null : xb.trim();
    }

    public String getCsrq() {
        return csrq;
    }

    public void setCsrq(String csrq) {
        this.csrq = csrq == null ? null : csrq.trim();
    }

    public String getGmsfhm() {
        return gmsfhm;
    }

    public void setGmsfhm(String gmsfhm) {
        this.gmsfhm = gmsfhm == null ? null : gmsfhm.trim();
    }

    public String getMz() {
        return mz;
    }

    public void setMz(String mz) {
        this.mz = mz == null ? null : mz.trim();
    }

    public String getCsgj() {
        return csgj;
    }

    public void setCsgj(String csgj) {
        this.csgj = csgj == null ? null : csgj.trim();
    }

    public String getGj() {
        return gj;
    }

    public void setGj(String gj) {
        this.gj = gj == null ? null : gj.trim();
    }

    public String getHjzzbh() {
        return hjzzbh;
    }

    public void setHjzzbh(String hjzzbh) {
        this.hjzzbh = hjzzbh == null ? null : hjzzbh.trim();
    }

    public String getJgqx() {
        return jgqx;
    }

    public void setJgqx(String jgqx) {
        this.jgqx = jgqx == null ? null : jgqx.trim();
    }

    public String getHjqh() {
        return hjqh;
    }

    public void setHjqh(String hjqh) {
        this.hjqh = hjqh == null ? null : hjqh.trim();
    }

    public String getHjxz() {
        return hjxz;
    }

    public void setHjxz(String hjxz) {
        this.hjxz = hjxz == null ? null : hjxz.trim();
    }

    public String getJwzrq() {
        return jwzrq;
    }

    public void setJwzrq(String jwzrq) {
        this.jwzrq = jwzrq == null ? null : jwzrq.trim();
    }

    public String getXzzbh() {
        return xzzbh;
    }

    public void setXzzbh(String xzzbh) {
        this.xzzbh = xzzbh == null ? null : xzzbh.trim();
    }

    public String getXzqh() {
        return xzqh;
    }

    public void setXzqh(String xzqh) {
        this.xzqh = xzqh == null ? null : xzqh.trim();
    }

    public String getXzxz() {
        return xzxz;
    }

    public void setXzxz(String xzxz) {
        this.xzxz = xzxz == null ? null : xzxz.trim();
    }

    public String getXzzrq() {
        return xzzrq;
    }

    public void setXzzrq(String xzzrq) {
        this.xzzrq = xzzrq == null ? null : xzzrq.trim();
    }

    public String getFwcs() {
        return fwcs;
    }

    public void setFwcs(String fwcs) {
        this.fwcs = fwcs == null ? null : fwcs.trim();
    }

    public String getZzmm() {
        return zzmm;
    }

    public void setZzmm(String zzmm) {
        this.zzmm = zzmm == null ? null : zzmm.trim();
    }

    public String getWhcd() {
        return whcd;
    }

    public void setWhcd(String whcd) {
        this.whcd = whcd == null ? null : whcd.trim();
    }

    public Short getSg() {
        return sg;
    }

    public void setSg(Short sg) {
        this.sg = sg;
    }

    public String getByzk() {
        return byzk;
    }

    public void setByzk(String byzk) {
        this.byzk = byzk == null ? null : byzk.trim();
    }

    public String getHyzk() {
        return hyzk;
    }

    public void setHyzk(String hyzk) {
        this.hyzk = hyzk == null ? null : hyzk.trim();
    }

    public String getXx() {
        return xx;
    }

    public void setXx(String xx) {
        this.xx = xx == null ? null : xx.trim();
    }

    public String getZw() {
        return zw;
    }

    public void setZw(String zw) {
        this.zw = zw == null ? null : zw.trim();
    }

    public String getZylb() {
        return zylb;
    }

    public void setZylb(String zylb) {
        this.zylb = zylb == null ? null : zylb.trim();
    }

    public String getSf() {
        return sf;
    }

    public void setSf(String sf) {
        this.sf = sf == null ? null : sf.trim();
    }

    public String getRylb() {
        return rylb;
    }

    public void setRylb(String rylb) {
        this.rylb = rylb == null ? null : rylb.trim();
    }

    public String getRybq() {
        return rybq;
    }

    public void setRybq(String rybq) {
        this.rybq = rybq == null ? null : rybq.trim();
    }

    public String getYwm() {
        return ywm;
    }

    public void setYwm(String ywm) {
        this.ywm = ywm == null ? null : ywm.trim();
    }

    public String getCsxz() {
        return csxz;
    }

    public void setCsxz(String csxz) {
        this.csxz = csxz == null ? null : csxz.trim();
    }

    public String getZjlx() {
        return zjlx;
    }

    public void setZjlx(String zjlx) {
        this.zjlx = zjlx == null ? null : zjlx.trim();
    }

    public String getZjhm() {
        return zjhm;
    }

    public void setZjhm(String zjhm) {
        this.zjhm = zjhm == null ? null : zjhm.trim();
    }

    public String getLxdh() {
        return lxdh;
    }

    public void setLxdh(String lxdh) {
        this.lxdh = lxdh == null ? null : lxdh.trim();
    }

    public String getZjxy() {
        return zjxy;
    }

    public void setZjxy(String zjxy) {
        this.zjxy = zjxy == null ? null : zjxy.trim();
    }

    public String getDjrzh() {
        return djrzh;
    }

    public void setDjrzh(String djrzh) {
        this.djrzh = djrzh == null ? null : djrzh.trim();
    }

    public String getDjrxm() {
        return djrxm;
    }

    public void setDjrxm(String djrxm) {
        this.djrxm = djrxm == null ? null : djrxm.trim();
    }

    public String getDjsj() {
        return djsj;
    }

    public void setDjsj(String djsj) {
        this.djsj = djsj == null ? null : djsj.trim();
    }

    public String getDjdw() {
        return djdw;
    }

    public void setDjdw(String djdw) {
        this.djdw = djdw == null ? null : djdw.trim();
    }

    public String getDjdwmc() {
        return djdwmc;
    }

    public void setDjdwmc(String djdwmc) {
        this.djdwmc = djdwmc == null ? null : djdwmc.trim();
    }

    public String getXgrzh() {
        return xgrzh;
    }

    public void setXgrzh(String xgrzh) {
        this.xgrzh = xgrzh == null ? null : xgrzh.trim();
    }

    public String getXgrxm() {
        return xgrxm;
    }

    public void setXgrxm(String xgrxm) {
        this.xgrxm = xgrxm == null ? null : xgrxm.trim();
    }

    public String getXgsj() {
        return xgsj;
    }

    public void setXgsj(String xgsj) {
        this.xgsj = xgsj == null ? null : xgsj.trim();
    }

    public String getXgdw() {
        return xgdw;
    }

    public void setXgdw(String xgdw) {
        this.xgdw = xgdw == null ? null : xgdw.trim();
    }

    public String getXgdwmc() {
        return xgdwmc;
    }

    public void setXgdwmc(String xgdwmc) {
        this.xgdwmc = xgdwmc == null ? null : xgdwmc.trim();
    }

    public String getSjly() {
        return sjly;
    }

    public void setSjly(String sjly) {
        this.sjly = sjly == null ? null : sjly.trim();
    }

    public String getZwbh() {
        return zwbh;
    }

    public void setZwbh(String zwbh) {
        this.zwbh = zwbh == null ? null : zwbh.trim();
    }

    public String getDnabh() {
        return dnabh;
    }

    public void setDnabh(String dnabh) {
        this.dnabh = dnabh == null ? null : dnabh.trim();
    }

    public String getXxjb() {
        return xxjb;
    }

    public void setXxjb(String xxjb) {
        this.xxjb = xxjb == null ? null : xxjb.trim();
    }

    public String getFwbh() {
        return fwbh;
    }

    public void setFwbh(String fwbh) {
        this.fwbh = fwbh == null ? null : fwbh.trim();
    }

    public String getFwdz() {
        return fwdz;
    }

    public void setFwdz(String fwdz) {
        this.fwdz = fwdz == null ? null : fwdz.trim();
    }
}